...
import { buildReportMessages, buildStep1Messages, buildStep2Messages, buildMessages } from '../server/src/services/promptBuilder';
...
describe('AI 解读 · 当前时间事实注入', () => {
  ...
  it('Step2 主路径（buildStep2Messages，流式解读实际用的）同样注入', () => {
    const msgs = buildStep2Messages('liuren', '未来一年运势如何', { dayGZ: '乙亥' } as any, '盘面解析文本');
    const user = msgs[msgs.length - 1].content as string;
    expect(user).toContain('【当前时间 · 事实】');
    expect(user).toContain(new Date().getFullYear() + ' 年');
  });

  it('简易解读（buildMessages）同样注入', () => {
    const msgs = buildMessages('tarot', '这段感情如何', { cards: [] } as any);
    const user = msgs[msgs.length - 1].content as string;
    expect(user).toContain('【当前时间 · 事实】');
  });
