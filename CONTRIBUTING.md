...
2. 小步提交，提交信息用中文、**保持简洁**：`type: 一句话`（type ∈ feat / fix / docs / chore / test / refactor）。**不要**在提交信息里写运营动作、内部过程或策略细节（如"为曝光优化""移除某工具痕迹"）——提交信息会在 GitHub 上公开展示，只描述代码变更本身。
3. 每个 PR 至少覆盖：
   - 相关测试（新增功能必须有测试）
...
- 测试中的"真实案例"仅保留在 git 忽略的 `tmp-test-*` 文件中，入库测试一律虚构/匿名
